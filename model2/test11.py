...
# X_train = pd.DataFrame(scaler.fit_transform(X_train[numeric_cols]), columns=numeric_cols)["url_length"].values.reshape(-1, 1)
# X_test = pd.DataFrame(scaler.transform(X_test[numeric_cols]), columns=numeric_cols)["url_length"].values.reshape(-1, 1)
X_train = pd.DataFrame(scaler.fit_transform(X_train[numeric_cols]), columns=numeric_cols)
X_test = pd.DataFrame(scaler.transform(X_test[numeric_cols]), columns=numeric_cols)
